...
export default function Portfolio() {
  ...
  return (
    <section className="relative py-20 md:py-32 bg-white dark:bg-black overflow-hidden border-t border-gray-100 dark:border-white/5">
      ...
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-gray-900 dark:text-white mb-5 tracking-tight">
            Projects We're Proud Of
          </h2>
          <p className="text-lg text-gray-500 dark:text-gray-400 max-w-xl mx-auto font-light">
          ...
          {filters.map((filter) => (
            ...
            className={`px-6 py-2.5 rounded-full text-sm font-bold uppercase tracking-wider transition-all duration-300 ${
                activeFilter === filter
                  ...
                  : "border border-gray-200 dark:border-white/[0.12] text-gray-500 dark:text-gray-400 hover:border-[#0022FF] hover:text-[#0022FF] dark:hover:border-white/25 dark:hover:text-white bg-transparent"
              }`}
            >
              {filter}
            </button>
        ...
        <motion.div layout className="relative">
          {/* Top border */}
          <div className="h-px bg-gray-200 dark:bg-white/10 mb-0" />
            ...
            {filtered.map((item, index) => {
              ...
              return (
                ...
                >
                  <Row
                    {...(linkProps as any)}
                    className={`group flex items-center justify-between gap-4 py-5 border-b border-gray-100 dark:border-white/8 transition-all duration-300 ${
                      item.link
                        ? "cursor-pointer hover:bg-gray-50 dark:hover:bg-white/[0.03] -mx-4 px-4 rounded-xl"
                        : "-mx-4 px-4"
                    }`}
                  >
                    {/* Index + name */}
                    <div className="flex items-center gap-5 min-w-0">
                      <span className="hidden sm:block text-xs font-mono text-gray-300 dark:text-white/25 w-6 shrink-0 tabular-nums font-bold">
                        {String(index + 1).padStart(2, "0")}
                      </span>
                      <h3
                        className={`text-lg sm:text-xl font-bold tracking-tight truncate transition-colors duration-300 ${
                          item.link
                            ? "text-gray-900 dark:text-white group-hover:text-[#00C6FF]"
                    ...
                    <div className="flex items-center gap-3 sm:gap-5 shrink-0">
                      <span className="hidden md:inline-block text-xs font-semibold px-3 py-1 rounded-full bg-gray-100 dark:bg-white/[0.08] text-gray-500 dark:text-gray-300 uppercase tracking-wider border border-gray-200 dark:border-white/[0.14]">
                        {item.category}
                      </span>
                      ...
                      {item.link ? (
                        <span className="w-8 h-8 rounded-full flex items-center justify-center bg-gray-100 dark:bg-white/8 text-gray-400 dark:text-gray-500 group-hover:bg-[#00C6FF] group-hover:text-white transition-all duration-300 shrink-0">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="flex flex-col sm:flex-row items-center justify-between gap-6 mt-12 pt-8 border-t border-gray-100 dark:border-white/10"
        >
          <p className="text-sm text-gray-400 dark:text-gray-500">
            <span className="font-bold text-gray-700 dark:text-white text-lg">
