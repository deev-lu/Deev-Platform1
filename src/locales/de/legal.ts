import type { legal as En } from "../en/legal";

export const legal: typeof En = {
  badge: "Rechtliche Hinweise",
  title: "AGB und",
  titleAccent: "Impressum",
  updated: "Zuletzt aktualisiert: Mai 2025",

  company: {
    heading: "Angaben zum Unternehmen",
    tradeName: "Handelsname",
    legalEntity: "Rechtsträger",
    directors: "Geschäftsführer",
    address: "Anschrift",
    email: "E-Mail",
    phone: "Telefon",
    vat: "USt-IdNr.",
    register: "Handelsregister",
  },

  s1: {
    title: "1. Leistungen",
    body: "Deev (Lux VR States Sàrl-s.) erbringt Leistungen einer Digitalagentur, insbesondere: Webdesign und Webentwicklung, KI-Integration, digitales Marketing, E-Commerce-Lösungen und zugehörige Beratung. Alle Leistungen unterliegen einem gesonderten Dienstleistungsvertrag zwischen Deev und dem Kunden.",
  },
  s2: {
    title: "2. Voraussetzungen",
    body: "Mit der Beauftragung unserer Leistungen bestätigen Sie, dass Sie mindestens 18 Jahre alt und geschäftsfähig sind. Unternehmen müssen ordnungsgemäß eingetragen sein. Deev behält sich vor, Projektanfragen nach eigenem Ermessen abzulehnen.",
  },
  s3: {
    title: "3. Bestellung und Zahlung",
    body: "Alle Preise verstehen sich in Euro und zuzüglich Mehrwertsteuer, sofern nicht anders angegeben. Vor Arbeitsbeginn ist eine Anzahlung (in der Regel 50%) fällig. Der Restbetrag wird mit Projektabschluss oder gemäß Projektvertrag fällig. Rechnungen sind innerhalb von 14 Tagen ab Ausstellung zahlbar. Bei Zahlungsverzug können nach luxemburgischem Recht Verzugszinsen anfallen.",
  },
  s4: {
    title: "4. Förderung durch SME Digital und SME AI",
    body: "Förderfähige luxemburgische KMU können staatliche Zuschüsse aus den Programmen SME Digital und SME AI erhalten, die 70% der förderfähigen Investitionskosten abdecken. Projekte müssen im förderfähigen Kostenrahmen liegen (3.000 € bis 25.000 €). Deev unterstützt bei der Antragstellung; die Bewilligung liegt jedoch im alleinigen Ermessen der zuständigen Behörde. Deev übernimmt keine Gewähr für die Bewilligung.",
  },
  s5: {
    title: "5. Preise und Änderungen",
    body: "Deev behält sich vor, die Preise jederzeit anzupassen. Preisänderungen gelten nicht für bereits vertraglich vereinbarte Projekte. Vom Kunden nach Vertragsschluss gewünschte Änderungen oder Erweiterungen des Leistungsumfangs können zusätzliche Kosten verursachen, die vor der Umsetzung schriftlich vereinbart werden.",
  },
  s6: {
    title: "6. Geistiges Eigentum",
    body: "Nach vollständiger Zahlung erhält der Kunde das uneingeschränkte Eigentum an allen individuell für sein Projekt erstellten Ergebnissen. Deev behält das Recht, die Arbeit im eigenen Portfolio zu zeigen, sofern nichts anderes schriftlich vereinbart wurde. Alle verwendeten Inhalte, Bibliotheken oder Software Dritter unterliegen weiterhin ihren jeweiligen Lizenzen.",
  },

  cookies: {
    title: "7. Cookies, Einwilligung und Tracking",
    intro:
      "Diese Website setzt eine kleine Zahl eigener Cookies und, ausschließlich mit Ihrer Einwilligung, Cookies von Google Analytics. Es gibt keine Consent-Plattform eines Dritten: Banner, Einstellungsdialog und der Nachweis Ihrer Auswahl sind Teil dieser Website, und es werden keinerlei Daten über Sie irgendwohin übermittelt, nur um Ihnen die Frage zu stellen.",
    necessaryHead: "Unbedingt erforderlich",
    necessaryBody:
      "Gesetzt auf der Rechtsgrundlage von Artikel 5(3) der ePrivacy-Richtlinie, der für technisch notwendige Vorgänge zur Erbringung des von Ihnen angeforderten Dienstes keine Einwilligung verlangt.",
    rowConsent: "Ihre Cookie-Auswahl und deren Nachweis: eine Referenz, das Datum und die Richtlinienversion.",
    rowTheme: "Merkt sich die von Ihnen gewählte helle oder dunkle Darstellung.",
    lifeConsent: "12 Monate",
    lifeTheme: "Bis zum Löschen",
    analyticsHead: "Statistik, nur mit Einwilligung",
    analyticsBody:
      "Wir verwenden Google Analytics 4, in der EU betrieben von Google Ireland Limited, um Besuche zu zählen und zu sehen, welche Seiten gelesen werden. Es läuft im Google Consent Mode. Vor Ihrer Auswahl und bei Ablehnung bleibt das Tag im Zustand der verweigerten Einwilligung: Es wird kein Cookie geschrieben und keine Kennung auf Ihrem Gerät gespeichert, und Google erhält nur ein cookieloses Signal, einschließlich Ihrer IP-Adresse und Ihres Browsertyps, das für aggregierte Statistiken verwendet wird. Wenn Sie zustimmen, werden die untenstehenden Cookies gesetzt.",
    rowGa: "Unterscheidet einen Besucher vom anderen, damit Besuche gezählt werden können.",
    rowGaProperty: "Hält den Zustand des aktuellen Besuchs für diese Property fest.",
    lifeGa: "2 Jahre",
    advertising:
      "Werbespeicherung, Anzeigenpersonalisierung und Nutzerdaten für Werbung sind dauerhaft abgelehnt, und wir setzen auf dieser Website keine Werbe- oder Remarketing-Produkte ein. Google kann die Daten außerhalb der EU verarbeiten; diese Übermittlungen stützen sich auf die Standardvertragsklauseln der Europäischen Kommission und auf ergänzende Maßnahmen von Google.",
    videoHead: "Eingebettete Videos",
    videoBody:
      "Der Marketingbereich enthält Videos, die bei YouTube gehostet werden. Der Player selbst wird erst geladen, wenn Sie auf Wiedergabe drücken, sodass vorher kein YouTube-Skript ausgeführt und kein Speicher Dritter auf Ihrem Gerät gesetzt wird. Das Standbild, das Sie zuvor sehen, ist das Vorschaubild des Videos selbst, das beim Erscheinen dieses Seitenbereichs vom Bildhost von Google (i.ytimg.com) angefordert wird: Diese Anfrage setzt keine Cookies, teilt Google aber mit, dass ein Browser diese Seite geladen hat, einschließlich Ihrer IP-Adresse. Wenn Sie auf Wiedergabe drücken, wird der Player von youtube-nocookie.com geladen, das das Video ohne Werbecookies ausliefert; YouTube kann dann speichern, was zur Wiedergabe und zur Zählung des Aufrufs nötig ist. Auf Wiedergabe zu drücken ist Ihre eigene Entscheidung.",
    recordHead: "Ihr Einwilligungsnachweis",
    recordBody1: "Wenn Sie sich entscheiden, speichern wir die Auswahl selbst, eine zufällige Referenz, Datum und Uhrzeit sowie die Version dieser Richtlinie, die Ihnen angezeigt wurde. Dieser Datensatz ist der nach Artikel 7(1) DSGVO erforderliche Nachweis der Einwilligung. Er liegt im Cookie",
    recordBody2: "auf Ihrem eigenen Gerät und wird im lokalen Speicher Ihres Browsers gespiegelt; er wird weder an uns noch an Dritte übermittelt. Er läuft nach zwölf Monaten ab, und wir fragen früher erneut, wenn sich diese Richtlinie ändert.",
    withdrawHead: "Auswahl ändern oder widerrufen",
    withdrawBody:
      "Der Widerruf ist so einfach wie die Einwilligung, wie Artikel 7(3) DSGVO es verlangt. Öffnen Sie den Dialog unten oder nutzen Sie „Cookie-Einstellungen“ im Fußbereich jeder Seite. Der Widerruf löscht den Nachweis, versetzt die Statistik zurück in den abgelehnten Zustand und zeigt das Banner erneut an. Sie können auch die Daten dieser Website in Ihrem Browser löschen, was dieselbe Wirkung hat.",
    manage: "Cookie-Einstellungen verwalten",
  },

  gdpr: {
    title: "8. Datenschutz und DSGVO",
    body1:
      "Deev verarbeitet personenbezogene Daten gemäß der Verordnung (EU) 2016/679 (DSGVO) und den geltenden luxemburgischen Datenschutzvorschriften. Über diese Website oder im Rahmen der Leistungserbringung erhobene personenbezogene Daten werden ausschließlich zur Erbringung unserer Leistungen, zur Kommunikation mit Kunden und zur Erfüllung gesetzlicher Pflichten verwendet. Daten werden niemals an Dritte verkauft.",
    body2a: "Sie haben das Recht auf Auskunft, Berichtigung und Löschung Ihrer personenbezogenen Daten. Um diese Rechte auszuüben, schreiben Sie uns an",
    body2b: ". Daten werden nur so lange aufbewahrt, wie es für die Zwecke ihrer Erhebung erforderlich ist oder wie es das Gesetz verlangt.",
    assistant:
      "Die Website bietet einen KI-Assistenten an. Was Sie dort eingeben, wird an Anthropic PBC übermittelt, die die Antwort als Auftragsverarbeiter auf Grundlage eines Auftragsverarbeitungsvertrags erzeugt; diese Verarbeitung findet außerhalb der Europäischen Union auf Basis der Standardvertragsklauseln der Europäischen Kommission statt. Deev zeichnet das Gespräch weder auf noch speichert es, und der Assistent hat keinen Zugriff auf Ihre Akte, Ihr Konto oder Kundendaten. Bitte geben Sie dort keine persönlichen oder vertraulichen Angaben ein: Nutzen Sie das Kontaktformular oder contact@deev.lu, wo ein Mensch mitliest. Antworten werden automatisch erzeugt, können falsch sein und stellen keine Beratung dar; nichts, was der Assistent sagt, ist Teil eines Angebots oder Vertrags.",
  },

  s9: {
    title: "9. Haftungsbeschränkung",
    body: "Die Gesamthaftung von Deev gegenüber einem Kunden aus oder im Zusammenhang mit einem Projekt übersteigt nicht die Summe der von diesem Kunden im Rahmen des betreffenden Projektvertrags gezahlten Honorare. Deev haftet nicht für mittelbare Schäden, Folgeschäden oder Nebenschäden. Keine Bestimmung dieser Bedingungen beschränkt die Haftung von Deev für Betrug oder grobe Fahrlässigkeit.",
  },
  s10: {
    title: "10. Anwendbares Recht und Gerichtsstand",
    body: "Diese Bedingungen und alle daraus entstehenden Streitigkeiten unterliegen dem Recht des Großherzogtums Luxemburg und werden nach diesem ausgelegt. Für alle Streitigkeiten sind ausschließlich die Gerichte der Stadt Luxemburg zuständig.",
  },
  s11: {
    title: "11. Kontakt",
    body: "Bei Fragen zu diesen Bedingungen wenden Sie sich bitte an:",
  },

  prevails: {
    title: "12. Sprache",
    body: "Diese Übersetzung wird zu Ihrer besseren Verständlichkeit bereitgestellt. Bei Abweichungen oder Widersprüchen zwischen dieser Fassung und der englischen Fassung ist die englische Fassung maßgeblich und allein vertraglich bindend. Diese Bedingungen sind ein Mustertext und wurden nicht anwaltlich geprüft; wir empfehlen Ihnen, sie vor einer Berufung darauf rechtlich prüfen zu lassen.",
  },

  back: "Zurück zu DEEV",
};
